		...
		var Plugin = {
			...
			dataRender: function(action) {
				...
				var afterGetData = function(result) {
					...
					$(datatable.wrap).removeClass(pfx + 'datatable--errors');
					// pagination enabled
			...
			insertData: function() {
				...
				if (datatable.dataSet.length === 0) {
					...
					util.addClass(errorSpan, pfx + 'datatable--errors');
					errorSpan.innerHTML = Plugin.getOption('translate.records.noRecords');
					tableBody.appendChild(errorSpan);
					$(datatable.wrap).addClass(pfx + 'datatable--errors ' + pfx + 'datatable--loaded');
					Plugin.spinnerCallback(false);
				}
			...
			getData: function() {
				...
				return $.ajax(ajaxParams).done(function(response, textStatus, jqXHR) {
					...
					$(datatable.tableBody).html($('<span/>').addClass(pfx + 'datatable--errors').html(Plugin.getOption('translate.records.noRecords')));
					$(datatable.wrap).addClass(pfx + 'datatable--errors ' + pfx + 'datatable--loaded');
					Plugin.spinnerCallback(false);
				}).always(function() {
				});
		...
		var Extension = {
			...
			initSelect: function() {
				...
				if (Extension.selectedAllRows && options.vars.requestIds) {
					if (!datatable.hasClass(pfx + 'datatable--errors')) {
					...
					if (!datatable.hasClass(pfx + 'datatable--errors') && $(datatable.tableBody).find('.' + pfx + 'checkbox--single > [type="checkbox"]').not(':checked').length < 1) {
						// set header select all checkbox checked
						$(datatable.tableHead).find('.' + pfx + 'checkbox--all > [type="checkbox"]').prop('checked', true);
					}
				}
        ...
        setTimeout(function() {
            $.ajax({
                ...
                error: function(res) {
                    ...
                    hideProgress();
                    KTUtil.addClass(target, resultClass);
                    KTUtil.setHTML(resultWrapper, '<span class="kt-quick-search__message">Connection errors. Pleae try again later.</div>');
                    showDropdown();
                    KTUtil.scrollUpdate(resultWrapper);
                }
            });
