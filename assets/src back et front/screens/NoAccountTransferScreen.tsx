...
const NoAccountTransferScreen = ({ route, navigation }: any) => {
    ...
    const [lastname, setLastname] = useState(params?.prefillData?.lastname || '');
    const [firstname, setFirstname] = useState(params?.prefillData?.firstname || '');
    ...
    const handleSubmit = async () => {
        if (!phoneNumber || !amount || !lastname) {
            Alert.alert(t.alerts.error, t.alerts.fillAllFields);
            return;
        }
        try {
            setLoading(true);
            const result = await homeService.previewNonAccTransfert({
                amount,
                beneficiary: {
    ...
    const handleConfirmTransfer = async (password: string) => {
        ...
        try {
            const result = await homeService.nonAccTransfert({
                amount,
                beneficiary: {
                    phone_number: phoneNumber,
                    firstname,
        ...
        //     setIsLoading(false);
        //     setIsConfirmVisible(false);
        //     console.log('Transfert confirmé avec le mot de passe:', password);
        // }, 2000);
    };
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                <View style={styles.formContainer}>
                    ...
                    <View style={styles.inputGroup}>
                        <Text style={styles.inputLabel}>{t.transfer.lastname}</Text>
                        <View style={styles.inputContainer}>
                            <Ionicons name="person-outline" size={20} color="#fcbf00" style={styles.inputIcon} />
                            <TextInput
                                style={styles.input}
                                placeholder="Nom"
                    ...
                    {/* Champ Prenom avec Label */}
                    <View style={styles.inputGroup}>
                        <Text style={styles.inputLabel}>{t.transfer.firstname}</Text>
                        <View style={styles.inputContainer}>
                            <Ionicons name="people-outline" size={20} color="#fcbf00" style={styles.inputIcon} />
                            <TextInput
                                style={styles.input}
                                placeholder="Prenom"
                                placeholderTextColor="#A0AEC0"
                    ...
                    {commission !== null && (
                        <View style={styles.commissionContainer}>
                            ...
                            {/* <Text style={styles.commissionNote}>(calcule sur le montant à transféré)</Text> */}
                        </View>
                    )}
            ...
            >
                {loading ? (
                    ...
                    <View style={styles.submitButtonContent}>
                        ...
                        <Text style={styles.submitButtonText}>Valider le transfert</Text>
                    </View>
                )}
            </TouchableOpacity>
        </KeyboardAvoidingView>
...
export default NoAccountTransferScreen;