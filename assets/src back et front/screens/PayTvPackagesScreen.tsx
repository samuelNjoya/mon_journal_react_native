...
const PaymentScreen1 = ({ route, navigation }: any) => {

    const { subscriptions, methodes } = route.params || {};
    const [password, setPassword] = useState('');
    const [selectedMethod, setSelectedMethod] = useState('SESAME');
    ...
    const [paymentMethodes, setPaymentMethodes] = useState<any>(methodes)
    ...
    const calculateTotal = useCallback(() => {
        return subscriptions.reduce((acc: number, item: any) => acc + item.total_with_reduction, 0)
    ...
    const handlePay = useCallback(async () => {
        ...
        try {
            ...
            const packages = subscriptions.map((x: any) => {
                return {
                    times_subscription: x.months,
                    id_tv_package: x.id_tv_package,
                    subscriber_number: x.subscriber_number,
                    phone_number: x.phone_number
                }
            })
            const result = await homeService.payTVPackages({
                times: subscriptions[0].months,
                op_reference: "SO_PYBCANAL",
                phone_number: subscriptions[0].phone_number,
                mypackages: packages,
                ...
                switch (selectedMethod) {
                    case 'SESAME':
                        navigation.navigate('PaymentSuccess', {
                            ...
                            balance_after: 0,
                            balance_before: 0,
                            transactionId: datas.transaction_code,
                            card: selectedMethod,
                            message: t.payment.tvSubscription,
                            timestamp: Date.now()
                        });
                        break;
                    case 'OM':
    ...
    }, [subscriptions, selectedMethod, password, fullPhoneNumber, checkPaymentStatus]);
    ...
    }, [password, requiresPhoneNumber, phoneNumber, subscriptions]);
                ...
                >
                    ...
                    <View style={styles.cardInfo}>
                        {/* <View style={styles.cardHeader}>
                            <Image
                                source={require('../../assets/logo_sesampayx.png')}
                                style={styles.cardImage}
                                resizeMode="contain"
                            />
                            <View style={styles.cardDetails}>
                                <Text style={styles.cardName}>{'Compte de privilège'}</Text>
                        ...
                        <View style={styles.amountContainer}>
                                        ...
                                        <View style={styles.modalSubscriptionInfo}>
                                            <Text>
                                                <Text style={{ fontSize: 12, fontFamily: "Poppins-Bold" }}>
                                                    {subscription.denomination}
                                                </Text>
                                                <Text style={{ fontSize: 10, fontFamily: "Poppins-Regular" }}>({subscription.title})</Text>
                                            </Text>
                                            ...
                                            <Text style={{ fontFamily: "Poppins-Regular", fontSize: 10 }}>
                                                {"Tel: "}{subscription.phone_number}
                                            </Text>
                                        </View>

                                        <View style={styles.quantitySelector}>
                                            <Text style={{ fontSize: 12, fontFamily: "Poppins-Bold" }}>{showBenaccPrice(subscription.total_with_reduction, 1)}</Text>
                                            <Text style={{ fontSize: 11, fontFamily: "Poppins-Regular", textDecorationLine: 'line-through', color: '#6c757d', }}>{showBenaccPrice(subscription.total, 1)}</Text>
                                        </View>
                                        {/* 
                            ...
                            <View style={styles.amountRow}>
                                ...
                                <Text style={styles.totalValue}>{showBenaccPrice(calculateTotal(), 1)}</Text>
                            </View>
