...
export default function ExpenseForm({ onSubmit, initialData = null, onCancel, }: ExpenseFormProps) {
  ...
  useEffect(() => {
    setCategoryItems(categories.map((cat) => ({ label: cat.name, value: cat.id })));
  }, [categories]);
  ...
  //  if (label.trim().length === 0 || !amount || isNaN(amount)) return;
    ...
    //  budget,
      ...
      date: initialData?.date || new Date().toISOString().split("T")[0], //permet en sorte de ne pas considerer la date du jour comme date lors de la modification
      isRecurring,
    };
  ...
  return (
    <View style={styles.container}>
      ...
      {selectorButton(
        budgets.find((b) => b.id === budget)?.name || "Choisir un budget", //conversion pourId Budget Number(budget))
        budgetModalVisible,
        () => setBudgetModalVisible(!budgetModalVisible)
      )}
      {/* reuse renderModalDropdown for budget (data shape : {label,value}) */}
      {renderModalDropdown(budgetModalVisible, setBudgetModalVisible, budgets as any[], budget, (value) => setBudget(value as number))}

      {/* Catégorie */}
      <Text style={styles.label}>Catégorie</Text>
      {selectorButton(
      //  categories.find((c) => c.id === category)?.name ?? "Choisir une catégorie",
        availableCategoriesBudget.find((c) => c.id === category)?.name ?? "Choisir une catégorie",
      ...
      {/* {renderModalDropdown(categoryModalVisible, setCategoryModalVisible, categories as any[], category, (value) => setCategory(value as number))} */}
      {renderModalDropdown(categoryModalVisible, setCategoryModalVisible, availableCategoriesBudget as any[], category, (value) => setCategory(value as number))}
